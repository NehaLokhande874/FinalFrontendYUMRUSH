...
<select  className='rounded-md border px-3 py-1 text-sm focus:outline-none focus:ring-2 border-[#ff4d2d] text-[#ff4d2d]' onChange={(e)=>handleUpdateStatus(data._id,data?.shopOrders?.shop?._id || data?.shopOrders?.shop,e.target.value)}>
...
<div className="mt-3 p-2 border rounded-lg text-sm bg-orange-50 gap-4">
   ...
   ):data?.shopOrders?.assignedDeliveryBoy?<div key="assigned">{data?.shopOrders?.assignedDeliveryBoy?.fullName}-{data?.shopOrders?.assignedDeliveryBoy?.mobile}</div>:<div>Waiting for delivery boy to accept</div>}
</div>}
