...
import { FaLeaf, FaMinus, FaPlus, FaHeart, FaRegHeart } from "react-icons/fa";
...
function FoodCard({ data, onClick }) {
  ...
  useEffect(() => {
    if (userData?.favourites) {
      setIsFav(userData.favourites.some(id => id.toString() === data._id?.toString()))
    }
  }, [userData, data._id])
  ...
  const handleFavourite = async (e) => {
    e.stopPropagation()
    try {
      const res = await axios.post(`${serverUrl}/api/user/favourite/${data._id}`, {}, { withCredentials: true })
      setIsFav(res.data.favourites.some(id => id.toString() === data._id?.toString()))
    } catch (err) {
      console.error(err)
    }
  }
  ...
  const handleAddToCart = (e) => {
    ...
    if (quantity > 0) {
      dispatch(addToCart({
        ...
        price: data.price,
        image: data.image,
        shop: data.shop?._id?.toString() || data.shop?.toString(),
        quantity,
    ...
    >
      <div className='relative w-full h-[180px] rounded-2xl overflow-hidden shadow-sm'>
        ...
        <button
          ...
          className='absolute top-3 left-3 bg-white/90 backdrop-blur-sm rounded-full p-1.5 shadow-sm z-10'
        >
          {isFav
            ? <FaHeart className='text-red-500 text-[12px]' />
            : <FaRegHeart className='text-gray-400 text-[12px]' />
          }
        </button>
      ...
      <div className="mt-3 flex flex-col px-1">
        ...
        <div className='flex items-center gap-1.5 mt-0.5 font-bold text-[15px]'>
          {itemRating.count > 0 ? (
            ...
            <span className="text-green-600 text-[11px] font-black uppercase tracking-wider bg-green-50 px-2 py-0.5 rounded-md border border-green-100">New</span>
          )}
        </div>
